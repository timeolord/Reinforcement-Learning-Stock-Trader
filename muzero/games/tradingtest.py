...
def makeList(value1, value2, value3, value4):
    ...
    for a in range(value1):
        for b in range(value2):
            for c in range(value3):
                for d in range(value4):
                    acts = (a, b, c, d)
                    num = np.ravel_multi_index(acts, (value1, value2, value3, value4))
                    actions.append(num)
    return actions
...
class MuZeroConfig:
    def __init__(self):
        ...
        # self.observation_shape = env.observation_space.shape
        actions = makeList(3, 4, env.positionsPerStock, len(env.stockList))
        # print(actions)
        self.action_space = actions  # Fixed list of all possible actions. You should only edit the length
        ...
        ### Self-Play
        self.num_workers = 14  # Number of simultaneous threads/workers self-playing to feed the replay buffer
        ...
        self.training_steps = 50_000  # Total number of training steps (ie weights update according to a batch)
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        ...
        self.dateList = [datetime.date(day=1, month=12, year=2019), datetime.date(day=30, month=1, year=2020),
        ...
        self.env = y_finance_env(self.stockList, datetime.date(day=7, month=4, year=2021), "1h", self.initialMoney)
        if seed is not None:
            self.env.seed(seed)
    ...
    def legal_actions(self):
        ...
        """
        actions = makeList(3, 4, self.env.positionsPerStock, len(self.env.stockList))
        return actions
    ...
    def action_to_string(self, action_number):
        ...
        """
        self.env.debug = True
        return f"{self.env.getNetWorth():.1f}$"
