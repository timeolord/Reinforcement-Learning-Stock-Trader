...
class SelfPlay:
    ...
    def continuous_self_play(self, shared_storage, replay_buffer, test_mode=False):
        while ray.get(shared_storage.get_info.remote("training_step")) < self.config.training_steps and not ray.get(
            ...
            if not test_mode:
                game_history = self.play_game(self.config.visit_softmax_temperature_fn(
                    trained_steps=ray.get(shared_storage.get_info.remote("training_step"))),
                    self.config.temperature_threshold, False, "self", 0)
                replay_buffer.save_game.remote(game_history, shared_storage)

            else:
                # Take the best action (no exploration) in test mode
                game_history = self.play_game(0, self.config.temperature_threshold, False,
    ...
    def play_game(
        ...
        with torch.no_grad():
            while not done and len(game_history.action_history) <= self.config.max_moves:
                assert (len(numpy.array(observation).shape) == 3), \
                ...
                assert (numpy.array(observation).shape == self.config.observation_shape), \
                ...
                if render:
                    print(f"Money: {self.game.action_to_string(action)}")
                    self.game.render()
...
class Node:
    ...
    def add_exploration_noise(self, dirichlet_alpha, exploration_fraction):
        ...
        for a, n in zip(actions, noise):
            val = self.children[a].prior * (1 - frac) + n * frac
            self.children[a].prior = val
